    ...
    private function getInputFromFileOrJson()
    {
        ...
        try {
            if ($this->getOption('fieldsFile')) {
                ...
                $jsonData = json_decode($fileContents, true);
                $primary_id = explode(':', $this->inputFields[0]['fieldInput'])[0];
                $fields = GeneratorFieldsInputUtil::validateFieldsFile($jsonData['fields']);
                foreach ($fields as $field) {
                    $fiel_name = explode(':', $field['fieldInput'])[0];
                    if ($primary_id !== $fiel_name && $fiel_name !== 'id') {
...
//                $this->inputFields = array_merge($this->inputFields, GeneratorFieldsInputUtil::validateFieldsFile($jsonData['fields']));
    ...
    {
        $primaryName = $this->dynamicVars['$PRIMARY_KEY_NAME$'];

        if (!empty($primaryName)) {
            $this->inputFields[] = GeneratorFieldsInputUtil::processFieldInput(
                $primaryName . ':increments',
                '',
                '',
                [
                    'searchable' => false,
